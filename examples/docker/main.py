...
def main() -> None:
    ...
    config = {
        ...
        "quote_currency": [
            x for x in os.getenv("quote_currency").split(",") if x != ""
        ],
        "target_quantity": [
            float(x) for x in os.getenv("target_quantity").split(",") if x != ""
        ],
        "quote_to_maintain": [
            float(x) for x in os.getenv("quote_to_maintain").split(",") if x != ""
        ],
        "margin": [float(x) for x in os.getenv("margin").split(",")],
        "lowest_buy_price": [
            float(x) for x in os.getenv("lowest_buy_price").split(",") if x != ""
        ],
    ...
    if config["telegram"]["token"] is None or not config["telegram"]["chat_id"] is None:
