...
The algorithm checks the price range every 6 hours by default. The margin, from how many percent price difference the algorithm becomes active, can also be adjusted via the configuration file.

Actions will be printed out on the command line and can also be sent to a telegram chat bot.
...
3. Start https://t.me/RawDataBot and write down your personal chat id
...
The file `/example/main.py` serves as an example on how to initialize and run this trading algorithm using the `.env` and `config.json` files (see <a href="https://github.com/btschwertfeger/Kraken-Rebalance-Bot" target="_blank">GitHub</a>).
...
| `demo`                  | `bool`                           | Trade or not trade. Set to True if you know what this algorithm does.                                                                                                   |
